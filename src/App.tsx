...
export default function App() {
    ...
    const selectedRustdesk = settings.machines.find(m => m.id === selectedMachine && m.protocol === 'rustdesk');
    ...
    async function beginConnection(m: Machine | undefined) {
        ...
        try {
            if (m.protocol === 'rustdesk') {
                await call('open_rustdesk_client', {id:m.id});
                setSelectedMachine(m.id); setPage('operate');
                setNotice(t('Cliente RustDesk aberto. Informe o ID e a senha na janela do cliente. A conexão é manual nesta versão.'));
                return;
            }
 ...
 {page === 'machines' && <><div className="page-heading"><div><div className="eyebrow">{t("AMBIENTES DE TRABALHO")}</div><h1>{t("Suas máquinas, em um só lugar.")}</h1><p>{t("Cadastre o destino e escolha a forma de acesso.")}</p><button className="button secondary" disabled={running} onClick={()=>void attempt(()=>call("open_rustdesk_client",{id:null}))}><ExternalLink size={16}/>{t("Experimentar cliente RustDesk")}</button></div><button className="button primary" onClick={() => setMachineModal('new')}><Plus size={16}/>{t(" Adicionar máquina")}</button></div><div className="card-grid">{settings.machines.map(m => <div className="panel machine-card" key={m.id}><div className="card-top"><span className="large-icon"><Computer size={24}/></span><span className="badge">{t(protocolNames[m.protocol])}</span></div><h3>{m.name}</h3><p>{m.protocol === 'rustdesk' ? `ID ${m.host}` : `${m.host}:${m.port}`}</p><div className="machine-meta"><span>{m.protocol === 'rustdesk' ? t('Cliente web · Controle manual') : m.username || t("Usuário a configurar")}</span><span>{m.protocol === 'rdp' ? t("Conector nativo") : m.protocol === 'rustdesk' ? t('Prévia manual') : t("Em desenvolvimento")}</span></div><div className="button-row"><button className="button primary" disabled={!['rdp','rustdesk'].includes(m.protocol) || connectingSaved || running} onClick={() => void beginConnection(m)}><Power size={14}/>{m.protocol === "rustdesk" ? t("Abrir cliente RustDesk") : t(" Conectar")}</button><button className="button secondary" onClick={() => setMachineModal(m)}>{t("Editar")}</button></div></div>)}<button className="add-card" onClick={() => setMachineModal('new')}><Plus size={26}/><strong>{t("Adicionar um Windows")}</strong><span>{t("Endereço, credenciais e conexão")}</span></button></div><div className="info-strip"><Network size={20}/><div><strong>{t("Primeiro conector: Microsoft RDP")}</strong><p>{t("RDP oferece controle com IA. RustDesk abre o cliente web para controle manual. NanoKVM permanece em desenvolvimento.")}</p></div></div></>}
...
function MachineForm({ machine, close, save, call }: {
    ...
    return <Modal title={machine ? t("Editar máquina") : t("Adicionar máquina Windows")} sub={t("Uma conexão salva para o seu próximo trabalho.")} close={close}><form onSubmit={e => { e.preventDefault(); setSaving(true); setSaveError(''); void save(m, password).catch(e => setSaveError(String(e))).finally(() => setSaving(false)); }}><div className="form-grid"><Label title={t("Nome da máquina")}><input required value={m.name} onChange={e => setM({ ...m, name: e.target.value })} placeholder={t("Ex.: Atendimento 01")}/></Label><Label title={t("Tipo de conexão")}><select value={m.protocol} onChange={e => {setM({ ...m, protocol: e.target.value as Machine['protocol'] });setPassword('');}}>{Object.entries(protocolNames).map(([k, v]) => <option key={k} value={k}>{v}{k === 'rustdesk' ? t(' · prévia manual') : k !== 'rdp' ? t(" · em desenvolvimento") : ''}</option>)}</select></Label></div>{m.protocol === 'rustdesk' ? <><Label title={t("ID RustDesk")} hint={t("Copie o ID exibido no RustDesk do Windows. A senha será solicitada pelo próprio cliente.")}><input required maxLength={64} pattern="[A-Za-z0-9_-]+" value={m.host} onChange={e=>setM({...m,host:e.target.value.replace(/\s/g,'')})} placeholder="123456789"/></Label><Label title={t("Endereço do cliente web")} hint={t("Use o cliente oficial ou a URL HTTPS do seu cliente web. Não inclua ID, senha ou parâmetros na URL.")}><input type="url" value={m.rustdeskWebUrl ?? ''} onChange={e=>setM({...m,rustdeskWebUrl:e.target.value})} placeholder="https://rustdesk.com/web/"/></Label><div className="info-strip">{t("RustDesk está disponível para controle manual. Captura e execução de planos por IA ainda não estão disponíveis neste conector.")}</div></> : <><div className="form-grid wide"><Label title={t("Endereço IP ou hostname")}><input required value={m.host} onChange={e => setM({ ...m, host: e.target.value })} placeholder="192.168.1.10"/></Label><Label title={t("Porta")}><input type="number" required min="1" max="65535" value={m.port} onChange={e => setM({ ...m, port: Number(e.target.value) })}/></Label></div><div className="form-grid"><Label title={t("Usuário do Windows")}><input value={m.username} onChange={e => setM({ ...m, username: e.target.value })} placeholder={t("operador")}/></Label><Label title={t("Domínio (opcional)")}><input value={m.domain} onChange={e => setM({ ...m, domain: e.target.value })}/></Label></div><Label title={t("Senha (opcional)")} hint={savedHere ? t("Senha salva no Chaves do macOS. Deixe em branco para manter; digite apenas para trocar.") : machine && sameTarget && saved === null ? statusError || t("Verificando se existe senha salva…") : t("Nenhuma senha salva para este endereço e usuário. Informe para conectar automaticamente nas próximas vezes.")}><input type="password" autoComplete="new-password" placeholder={savedHere ? t("Senha salva · não exibida") : t("Digite para salvar no Chaves")} value={password} onChange={e => setPassword(e.target.value)}/></Label><Label title={t("Impressão digital do certificado (opcional)")} hint={t("Para certificado não reconhecido, copie a impressão digital confirmada com o administrador. Não desativa a validação TLS.")}><input value={m.fingerprint} onChange={e => setM({ ...m, fingerprint: e.target.value })} placeholder={t("SHA-256 exibido na primeira conexão")}/></Label></>}{saveError && <div className="login-error" role="alert">{systemText(saveError)}</div>}<div className="modal-actions"><button type="button" className="button secondary" onClick={close}>{t("Cancelar")}</button><button disabled={saving} className="button primary"><Check size={15}/>{t(" Salvar máquina")}</button></div></form></Modal>;
}
