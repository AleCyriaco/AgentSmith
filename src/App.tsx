...
export default function App() {
    ...
    const [selectedMachine, setSelectedMachine] = useState(''), [selectedRun, setSelectedRun] = useState(''), [instructions, setInstructions] = useState(''), [notice, setNotice] = useState(''), [error, setError] = useState(''), [busy, setBusy] = useState(false), [manual, setManual] = useState(false), [focusRdp, setFocusRdp] = useState(false), [windowBusy, setWindowBusy] = useState(false);
    ...
    async function saveModel(profile: Profile, secret: string) { const profiles = [...settings.profiles.filter(p => p.id !== profile.id), profile]; const routes = { ...settings.routes }; if (!routes.planner.length)
        ...
        if (profile.vision && !routes[role].length)
 ...
 <section className="panel history-panel"><div className="panel-head"><h3>{t("Histórico de trabalho ")}<span className="count">{runs.length}</span></h3><span className="muted">{t("Progresso salvo neste Mac")}</span></div>{runs.length ? <div className="history-content"><div className="run-list">{runs.map(r => <button className={active?.id === r.id ? 'selected' : ''} key={r.id} onClick={() => setSelectedRun(r.id)}><Activity size={15}/><span>{r.title}</span><small>{t(statuses[r.status])}</small></button>)}</div><div className="run-log">{active?.log.slice(-8).map((line, i) => <div key={i}><span className="log-dot"/>{line}</div>)}</div></div> : <div className="history-empty"><Activity size={17}/><span>{t("As ações e verificações das suas tarefas vão aparecer aqui.")}</span></div>}</section></div></div>}
...
function PerformanceForm({ value, close, save }: {
 ...
 <div className="performance-presets">{[{ name: t("Ágil"), captureIntervalMs: 150, postActionDelayMs: 250, visionMaxWidth: 1280 }, { name: t("Equilibrado"), ...defaultPerformance }, { name: t("Econômico"), captureIntervalMs: 1000, postActionDelayMs: 1200, visionMaxWidth: 1600 }].map(({ name, ...v }) => <button type="button" className="button secondary" key={name} onClick={() => setP(old=>({...old,...v,nativeOcr:old.nativeOcr,allowCrops:old.allowCrops}))}>{name}</button>)}</div>
 <Label title={t("Intervalo entre capturas")} hint={t("100 a 2.000 ms. Menor intervalo deixa a visualização mais fluida e usa mais CPU.")}><input aria-label={t("Intervalo entre capturas em milissegundos")} type="number" required min="100" max="2000" step="50" value={p.captureIntervalMs} onChange={e => setP({ ...p, captureIntervalMs: Number(e.target.value) })}/><small>{(1000 / p.captureIntervalMs).toFixed(1)}{t(" capturas por segundo · Atualização da interface acompanha esse ritmo.")}</small></Label>
 <Label title={t("Pausa após cada ação")} hint={t("100 a 3.000 ms. O aplicativo também aguarda uma nova imagem antes de continuar.")}><input aria-label={t("Pausa após cada ação em milissegundos")} type="number" required min="100" max="3000" step="50" value={p.postActionDelayMs} onChange={e => setP({ ...p, postActionDelayMs: Number(e.target.value) })}/></Label>
...
function ModelForm({ vendor, profile, close, save, call, attempt }: {
 ...
 <label className="checkbox-row"><input type="checkbox" checked={p.vision} onChange={e => setP({ ...p, vision: e.target.checked })}/><span><strong>{t("Este modelo aceita imagens")}</strong><small>{t("Habilita o perfil para operar e verificar a tela remota. O teste de conexão verifica a resposta de texto.")}</small></span></label>
