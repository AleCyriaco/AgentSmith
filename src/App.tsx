...
import { emptySettings, defaultDisplay, defaultPerformance, type PerformanceSettings, type DisplaySettings, type SimplexStatus, type Settings, type Machine, type Profile, type Run, type Snapshot, type SessionInfo, type Vendor } from './types';
...
const pages = [{ id: 'operate', label: 'Central de operação', icon: Monitor }, { id: 'machines', label: 'Máquinas', icon: Computer }, { id: 'models', label: 'Provedores e modelos', icon: Cpu }, { id: 'routes', label: 'Roteamento de IA', icon: Route }, { id: 'alerts', label: 'Avisos e decisões', icon: BellRing }, { id: 'architecture', label: 'Como funciona', icon: Layers3 }];
...
function SecondFactorForm({ machine, canTrust, close, connect }: {
    machine: Machine;
    canTrust: boolean;
    close: () => void;
...
function OperatorChannel({ call, attempt }: {
    ...
    return <><div className="page-heading"><div><div className="eyebrow">{t("AVISOS E DECISÕES")}</div><h1>{t("Smith fala com você quando precisa.")}</h1><p>{t("Um canal SimpleX no seu próprio servidor. Recebe o desfecho de cada tarefa e, quando uma para, pergunta o que fazer — você responde do celular.")}</p></div></div>
 <div className="panel"><div className="panel-head"><div><span className="tiny-label">{t("PASSO 1")}</span><h3>{t("Seu servidor SimpleX")}</h3></div><ShieldCheck size={19}/></div>
  <p>{t("Cole o endereço smp:// do servidor que você mantém. Ele contém uma senha, então é guardado no Chaves do macOS e nunca aparece nas configurações.")}</p>
  <Label title={t("Endereço do servidor")} hint={t("Só o seu servidor é usado. AgentSmith nunca recorre aos servidores públicos do SimpleX.")}><input value={server} onChange={e => { setServer(e.target.value); setSent(false); }} placeholder="smp://…@100.x.y.z" autoComplete="off" spellCheck={false}/></Label>
...
function RustdeskDefaults({ settings, persist }: {
    ...
    const current = settings.rustdesk ?? { server: '', key: '' };
    const [server, setServer] = useState(current.server);
    const [key, setKey] = useState(current.key);
    const [saving, setSaving] = useState(false);
    const [saved, setSaved] = useState(false);
    const [error, setError] = useState('');
    const dirty = server.trim() !== current.server || key.trim() !== current.key;
    function save() {
        setSaving(true);
        setError('');
        void persist({ ...settings, rustdesk: { server: server.trim(), key: key.trim() } })
            .then(() => setSaved(true))
            .catch(e => setError(String(e)))
            .finally(() => setSaving(false));
    }
    return <div className="panel"><div className="panel-head"><div><span className="tiny-label">RustDesk</span><h3>{t("Servidor padrão")}</h3></div><Network size={19}/></div><p>{t("Vale para toda máquina RustDesk que não indicar um servidor próprio. Em branco, usa o servidor público do RustDesk.")}</p><div className="form-grid wide"><Label title={t("Servidor de ID")} hint={t("O mesmo endereço configurado no RustDesk em Settings → Network → ID/Relay server.")}><input value={server} onChange={e => { setServer(e.target.value); setSaved(false); }} placeholder="rs.example.com"/></Label><Label title={t("Chave do servidor")} hint={t("A chave pública em base64 exibida junto do servidor de ID. Sem ela AgentSmith não autentica a máquina e não abre a sessão.")}><input value={key} onChange={e => { setKey(e.target.value); setSaved(false); }} placeholder="base64"/></Label></div>{error && <div className="login-error" role="alert">{systemText(error)}</div>}<div className="button-row"><button className="button primary" disabled={saving || !dirty} onClick={save}><Check size={15}/>{saving ? t("Salvando…") : saved && !dirty ? t("Salvo") : t(" Salvar servidor")}</button></div></div>;
}
...
export default function App() {
    ...
    const [modelModal, setModelModal] = useState<{
        vendor: Vendor;
        profile?: Profile;
    } | null>(null), [machineModal, setMachineModal] = useState<Machine | 'new' | null>(null), [connectModal, setConnectModal] = useState<Machine | null>(null), [secondFactorModal, setSecondFactorModal] = useState<{machine: Machine; canTrust: boolean} | null>(null);
    ...
    async function saveMachine(m: Machine, password: string) { await persist({ ...settings, machines: [...settings.machines.filter(x => x.id !== m.id), m] }); if (password && m.protocol === 'rdp')
    ...
    async function beginConnection(m: Machine | undefined) {
        ...
        try {
            ...
            if (outcome.startsWith('second-factor')) {
                setSecondFactorModal({ machine: m, canTrust: outcome === 'second-factor' });
                return;
            }
    ...
    async function connectWithSecondFactor(code: string, trust: boolean) {
        if (!secondFactorModal)
            return;
        const m = secondFactorModal.machine;
 ...
 {page === 'alerts' && <OperatorChannel call={call} attempt={attempt}/>}
 {page === 'machines' && <><div className="page-heading"><div><div className="eyebrow">{t("AMBIENTES DE TRABALHO")}</div><h1>{t("Suas máquinas, em um só lugar.")}</h1><p>{t("Cadastre o destino e escolha a forma de acesso.")}</p><button className="button secondary" disabled={running} onClick={()=>void attempt(()=>call("open_rustdesk_client",{id:null}))}><ExternalLink size={16}/>{t("Experimentar cliente RustDesk")}</button></div><button className="button primary" onClick={() => setMachineModal('new')}><Plus size={16}/>{t(" Adicionar máquina")}</button></div><div className="card-grid">{settings.machines.map(m => <div className="panel machine-card" key={m.id}><div className="card-top"><span className="large-icon"><Computer size={24}/></span><span className="badge">{t(protocolNames[m.protocol])}</span></div><h3>{m.name}</h3><p>{m.protocol === 'rustdesk' ? `ID ${m.host}` : `${m.host}:${m.port}`}</p><div className="machine-meta"><span>{m.protocol === 'rustdesk' ? (m.rustdeskServer || settings.rustdesk?.server || t('Servidor público RustDesk')) : m.username || t("Usuário a configurar")}</span><span>{['rdp','rustdesk'].includes(m.protocol) ? t("Conector nativo") : t("Em desenvolvimento")}</span></div><div className="button-row"><button className="button primary" disabled={!['rdp','rustdesk'].includes(m.protocol) || connectingSaved || running} onClick={() => void beginConnection(m)}><Power size={14}/>{t(" Conectar")}</button>{m.protocol === 'rustdesk' && <button className="button secondary" disabled={running} onClick={() => void attempt(() => call("open_rustdesk_client", {id:m.id}))}><ExternalLink size={14}/>{t("Cliente manual")}</button>}<button className="button secondary" onClick={() => setMachineModal(m)}>{t("Editar")}</button></div></div>)}<button className="add-card" onClick={() => setMachineModal('new')}><Plus size={26}/><strong>{t("Adicionar um Windows")}</strong><span>{t("Endereço, credenciais e conexão")}</span></button></div><RustdeskDefaults settings={settings} persist={persist}/><div className="info-strip"><Network size={20}/><div><strong>{t("Primeiro conector: Microsoft RDP")}</strong><p>{t("RDP e RustDesk oferecem controle com IA. O cliente web RustDesk continua disponível para controle manual. NanoKVM permanece em desenvolvimento.")}</p></div></div></>}
 ...
 {connectModal && <ConnectForm machine={connectModal} close={() => setConnectModal(null)} connect={connectWithPassword}/>}{secondFactorModal && <SecondFactorForm machine={secondFactorModal.machine} canTrust={secondFactorModal.canTrust} close={() => setSecondFactorModal(null)} connect={connectWithSecondFactor}/>}
 </div>;
}
...
function MachineForm({ machine, close, save, call }: {
    ...
    useEffect(() => { if (!machine || machine.protocol !== "rdp")
    ...
    return <Modal title={machine ? t("Editar máquina") : t("Adicionar máquina Windows")} sub={t("Uma conexão salva para o seu próximo trabalho.")} close={close}><form onSubmit={e => { e.preventDefault(); setSaving(true); setSaveError(''); void save(m, password).catch(e => setSaveError(String(e))).finally(() => setSaving(false)); }}><div className="form-grid"><Label title={t("Nome da máquina")}><input required value={m.name} onChange={e => setM({ ...m, name: e.target.value })} placeholder={t("Ex.: Atendimento 01")}/></Label><Label title={t("Tipo de conexão")}><select value={m.protocol} onChange={e => {setM({ ...m, protocol: e.target.value as Machine['protocol'] });setPassword('');}}>{Object.entries(protocolNames).map(([k, v]) => <option key={k} value={k}>{v}{['rdp','rustdesk'].includes(k) ? '' : t(" · em desenvolvimento")}</option>)}</select></Label></div>{m.protocol === 'rustdesk' ? <><Label title={t("ID RustDesk")} hint={t("Copie o ID exibido no RustDesk do Windows.")}><input required maxLength={64} pattern="[A-Za-z0-9_-]+" value={m.host} onChange={e=>setM({...m,host:e.target.value.replace(/\s/g,'')})} placeholder="123456789"/></Label><Label title={t("Senha permanente (opcional)")} hint={savedHere ? t("Senha salva no Chaves do macOS. Deixe em branco para manter; digite apenas para trocar.") : t("Defina uma senha permanente no RustDesk do Windows e informe aqui. Sem ela, cada sessão depende de aprovação manual na máquina.")}><input type="password" autoComplete="new-password" placeholder={savedHere ? t("Senha salva · não exibida") : t("Digite para salvar no Chaves")} value={password} onChange={e => setPassword(e.target.value)}/></Label><div className="form-grid wide"><Label title={t("Servidor próprio (opcional)")} hint={t("Sobrepõe o servidor padrão só para esta máquina. Em branco, usa o padrão.")}><input value={m.rustdeskServer ?? ''} onChange={e=>setM({...m,rustdeskServer:e.target.value.trim()})} placeholder="rs.example.com"/></Label><Label title={t("Chave do servidor (opcional)")} hint={t("Chave pública em base64 do servidor indicado acima.")}><input value={m.rustdeskKey ?? ''} onChange={e=>setM({...m,rustdeskKey:e.target.value.trim()})} placeholder="base64"/></Label></div><Label title={t("Endereço do cliente web (opcional)")} hint={t("Usado apenas pelo cliente manual, não pela sessão com IA. Não inclua ID, senha ou parâmetros na URL.")}><input type="url" value={m.rustdeskWebUrl ?? ''} onChange={e=>setM({...m,rustdeskWebUrl:e.target.value})} placeholder="https://rustdesk.com/web/"/></Label><div className="info-strip">{t("A sessão só abre se a identidade da máquina for autenticada pelo servidor. AgentSmith não conecta sem criptografia.")}</div></> : <><div className="form-grid wide"><Label title={t("Endereço IP ou hostname")}><input required value={m.host} onChange={e => setM({ ...m, host: e.target.value })} placeholder="192.168.1.10"/></Label><Label title={t("Porta")}><input type="number" required min="1" max="65535" value={m.port} onChange={e => setM({ ...m, port: Number(e.target.value) })}/></Label></div><div className="form-grid"><Label title={t("Usuário do Windows")}><input value={m.username} onChange={e => setM({ ...m, username: e.target.value })} placeholder={t("operador")}/></Label><Label title={t("Domínio (opcional)")}><input value={m.domain} onChange={e => setM({ ...m, domain: e.target.value })}/></Label></div><Label title={t("Senha (opcional)")} hint={savedHere ? t("Senha salva no Chaves do macOS. Deixe em branco para manter; digite apenas para trocar.") : machine && sameTarget && saved === null ? statusError || t("Verificando se existe senha salva…") : t("Nenhuma senha salva para este endereço e usuário. Informe para conectar automaticamente nas próximas vezes.")}><input type="password" autoComplete="new-password" placeholder={savedHere ? t("Senha salva · não exibida") : t("Digite para salvar no Chaves")} value={password} onChange={e => setPassword(e.target.value)}/></Label><Label title={t("Impressão digital do certificado (opcional)")} hint={t("Para certificado não reconhecido, copie a impressão digital confirmada com o administrador. Não desativa a validação TLS.")}><input value={m.fingerprint} onChange={e => setM({ ...m, fingerprint: e.target.value })} placeholder={t("SHA-256 exibido na primeira conexão")}/></Label></>}{saveError && <div className="login-error" role="alert">{systemText(saveError)}</div>}<div className="modal-actions"><button type="button" className="button secondary" onClick={close}>{t("Cancelar")}</button><button disabled={saving} className="button primary"><Check size={15}/>{t(" Salvar máquina")}</button></div></form></Modal>;
}
