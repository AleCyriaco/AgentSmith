...
export default function SessionPanel(p: Props) {
    ...
    if (p.machine?.protocol === 'rustdesk') return <section className="panel session-panel" aria-label={t("Cliente RustDesk") }><div className="rdp-toolbar"><div className="rdp-identity"><Monitor size={18}/><div><span className="tiny-label">RustDesk · {t("Controle manual")}</span><h3>{p.machine.name}</h3></div></div></div><div className="session-controls"><select aria-label={t("Máquina selecionada")} value={p.selectedMachine} disabled={running} onChange={e=>p.select(e.target.value)}>{p.machines.map(m=><option key={m.id} value={m.id}>{m.name}</option>)}</select><button className="button primary" disabled={running || p.connecting} onClick={p.connect}><ExternalLink size={16}/>{t("Abrir cliente RustDesk")}</button></div><div className="screen-empty" style={{minHeight:300}}><Monitor size={42}/><h3>{t("Cliente web em janela própria")}</h3><p>{t("RustDesk está disponível para controle manual. Captura e execução de planos por IA ainda não estão disponíveis neste conector.")}</p><label className="field"><span>{t("ID RustDesk")}</span><input readOnly value={p.machine.host} onFocus={e=>e.target.select()}/></label><p>{t("Copie o ID e informe a senha diretamente no cliente. Abrir a janela não confirma conexão com o Windows.")}</p></div></section>;
  ...
  <div className="rdp-toolbar">
   <div className="rdp-identity"><Monitor size={18}/><div><span className="tiny-label">{t("CONEXÃO · ")}{p.machine?.protocol === 'rdp' ? 'Microsoft RDP' : t("Windows remoto")}</span><h3>{p.machine?.name ?? t("Seu próximo posto de trabalho")}</h3></div></div>
