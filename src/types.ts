...
export type SimplexStatus={active:boolean;address:string;contacts:number;message:string;local:boolean;pairingUntil:number;qr:string;host:{phase:string;message:string;serverAddress:string;serverQr:string}};
...
export type Run = {requireApproval?:boolean;repetition?:import('./repetition').RepeatState|null;progress?:{message:string;startedAt:number}|null;id:string; title:string; machineId:string; instructions:string; steps:Step[]; status:string; log:string[]; actionCount:number; updatedAt:number};
