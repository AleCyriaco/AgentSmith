...
**Preview 0.14.0** · Current build target: **Apple Silicon and macOS 26+**. RDP and RustDesk both carry AI sessions; the RustDesk transport has been verified against one live machine, and its second-factor flow is not yet reachable from the interface. Windows as the controller and NanoKVM connectors are not implemented yet.
